...
export const Hero = () => {
  ...
  return (
    <section className="flex mb-20 min-h-screen w-full max-w-page mx-auto isolate bg-fixed">
      ...
      <div className="pt-navbar py-10 px-4 flex-col gap-4 | sm:px-12 | lg:bg-transparent">
        <h1
        ...
        >
          <span className="">BLAZINGLY</span>{" "}
          <span className="flex">
            FAST
            <FaChevronRight className=" text-theme  ml-10 | md:inline-flex md:ml-20" />
            <FaChevronRight className=" text-theme -ml-8 | md:inline-flex md:-ml-10 | lg:-ml-20" />
            <FaChevronRight className=" text-theme -ml-8 | md:inline-flex md:-ml-10 | lg:-ml-20" />
          </span>{" "}
          <span className="">WEBSITES.</span>
        </h1>
        <p className="flex flex-col text-lg max-w-[60ch] text-left backdrop-blur-sm rounded-2xl | lg:opacity-0 mt-8 lg:animate-[fade-in_750ms_linear_750ms_forwards]">
      ...
      <dialog ref={dialogRef} open={isDialogOpen}>
        <div className="fixed inset-0 grid place-content-center bg-black bg-opacity-50 overflow-hidden">
          <button
            className="bg-gray-800 hover:bg-(--theme) outline outline-black rounded-full py-1 px-4 w-fit flex items-center gap-4 mb-2 ml-auto"
            onClick={closeDialog}
          >
            Close
            <RiCloseFill />
